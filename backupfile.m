%potential_map.m file, 18/07/20 10:50am, before added vectors to read end
%index of all values
...
%Added purely for debugging so all values could be printed at end of code
x_unit_coord = zeros(size(x_values));
y_unit_coord = zeros(size(y_values));
x_index = zeros(size(x_values));
y_index = zeros(size(y_values));

for i=1:length(x_values)
    x_unit_coord(i) = rem(x_values(i),unitcell.celldim(1));
    while x_unit_coord(i) < 0 %account for negative remainders
        x_unit_coord(i) = x_unit_coord(i) + unitcell.celldim(1);
    end
    %To remove gaps at cell edges: (add for y if necessary)
    if round(x_unit_coord(i),4) == round(unitcell.celldim(1), 4)
        x_unit_coord(i) = 0;
    end
    x_index(i) = find(round(unitcell.x, 4) == round(x_unit_coord(i),4));
    for j=1 : length(y_values) %yeah I know this isn't the prettiest way of doing it
        %disp([i,j])
        y_unit_coord(j) = rem(y_values(j),unitcell.celldim(2));
        while y_unit_coord(j) < 0 %account for negative remainders
            y_unit_coord(j) = y_unit_coord(j) + unitcell.celldim(2),
        end
        if y_unit_coord(j) < 0
            disp('f*ck')
        end
        %to find position in original unit cell
        y_index(j) = find(round(unitcell.y,4) == round(y_unit_coord(j),4));
        try
            potential(i,j) = PotMatrix(x_index(i), y_index(j));
        catch
            %warning('Indexing outside potential, value set to NaN')
            potential(i,j) = NaN;
        end
